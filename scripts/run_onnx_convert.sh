...
if [ "$#" -ne 2 ] ; then
    echo "run_onnx_convert.sh <model_root_path> <model_class> -- requires 2 arguments (model_root_path & model_class)"
    echo "-- <model_class> can be 'GPT2LMHeadModel', 'GPT2LMHeadModel_NoPadding' or 'GPT2LMHeadModel_BeamSearchStep'"
    exit
fi
