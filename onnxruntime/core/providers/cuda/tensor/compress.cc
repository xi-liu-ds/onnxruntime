...
Status Compress::ComputeInternal(OpKernelContext* ctx) const {
  ...
  CUDA_RETURN_IF_ERROR(CompressCalcPrefixSumTempStorageBytes(Stream(), reinterpret_cast<const int8_t*>(condition_data), condition_cumulative_sum, valid_condition_length, temp_storage_bytes));
  auto temp_buffer = GetScratchBuffer<uint8_t>(temp_storage_bytes);
  auto d_temp_storage = temp_buffer.get();
  CUDA_RETURN_IF_ERROR(CompressInclusivePrefixSum(Stream(), d_temp_storage, temp_storage_bytes, reinterpret_cast<const int8_t*>(condition_data), condition_cumulative_sum, valid_condition_length));
