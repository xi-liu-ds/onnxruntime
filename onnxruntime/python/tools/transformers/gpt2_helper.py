...
class MyGPT2LMHeadModel_BeamSearchStep(GPT2LMHeadModel):
    ...
    def forward(
        ...
        *past, 
        ...
        prev_step_results = prev_step_results.view(self.batch_size, -1,  prev_step_results.size(-1))
        prev_step_results = prev_step_results.gather(1, selected_input_seq.unsqueeze(-1).repeat(1, 1, prev_step_results.size(-1)))
        ...
        prev_step_scores = prev_step_scores.view(self.batch_size, -1,  prev_step_scores.size(-1))
        prev_step_scores = prev_step_scores.gather(1, selected_input_seq.unsqueeze(-1).repeat(1, 1, prev_step_scores.size(-1)))
        ...
        return (
            ...
            current_step_results.view(self.batch_size * self.beam_size, -1), 
            current_step_scores.view(self.batch_size * self.beam_size, -1)
        )
...
class Gpt2Inputs:
    ...
    def to_kwargs(self):
        return self.input_ids, self.position_ids, self.attention_mask, self.beam_select_idx, self.input_log_probs, self.input_unfinished_sents, self.prev_step_results, self.prev_step_scores, self.past

    def to_tuple(self) -> Tuple:
        return tuple(v for v in [self.input_ids, self.position_ids, self.attention_mask, self.beam_select_idx, self.input_log_probs, self.input_unfinished_sents, self.prev_step_results, self.prev_step_scores, self.past] if v is not None)
...
class Gpt2Helper:
    ...
    def get_dummy_inputs(batch_size: int,
        ...
        """
        # for non-beam search models, no beam size needed
        if not has_beam_search:
            beam_size = 1
        ...
        if has_beam_search:
            ...
            prev_step_results = torch.randint(low=0,
                                  high=vocab_size - 1,
                                  size=(batch_size, sequence_length),
                                  dtype=torch.int64,
                                  device=device)
    ...
    def get_output_shapes(batch_size: int,
        ...
        if has_beam_search:
            ...
            output_shapes["current_step_scores"] = [batch_size, beam_size, vocab_size]
    ...
    def export_onnx(model,
        ...
        if has_beam_search:
            output_names += ["output_selected_indices", "output_log_probs", "output_unfinished_sents", "current_step_results", "current_step_scores"]
